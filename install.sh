...
build_mono_collector() {
	...
	if [ -f "libpince/libmono_collector/mono_collector_x64.so" ] && [ -f "libpince/libmono_collector/mono_collector_x86.so" ] && [ -f "libpince/libmono_collector/mono_collector_wine_x64.dll" ] && [ -f "libpince/libmono_collector/mono_collector_wine_x86.dll" ] && [ -z "$PINCE_LIB_ONLY" ]; then
	...
	(
		...
		"$ZIG" build -Doptimize=ReleaseFast -Dtarget=x86_64-windows-gnu || return 1
		cp --preserve zig-out/bin/mono_collector.dll ../libpince/libmono_collector/mono_collector_wine_x64.dll || return 1
		"$ZIG" build -Doptimize=ReleaseFast -Dtarget=x86-windows-gnu || return 1
		cp --preserve zig-out/bin/mono_collector.dll ../libpince/libmono_collector/mono_collector_wine_x86.dll || return 1
	) || return 1
	return 0
}
