...
class ParseAndEval(gdb.Command):
    ...
    def invoke(self, arg, from_tty):
        ...
        try:
            ...
            logger.exception("Expression: %s\nCast type: %s", expression, str(cast))
            parsed_value = None
...
class GetStackTraceInfo(gdb.Command):
    ...
    def invoke(self, arg, from_tty):
        ...
        if not stack_pointer:
            logger.warning("Cannot get the value of $%s", sp_register)
...
class GetStackInfo(gdb.Command):
    ...
    def invoke(self, arg, from_tty):
        ...
        if not sp_address:
            logger.warning("Cannot get the value of $%s", stack_register)
        ...
        with open(gdbutils.mem_file, "rb") as FILE:
            try:
                ...
                logger.exception("Cannot accesss the memory at %s", hex(sp_address))
                send_to_pince(stack_info_list)
                return
            for index in range(int(4096 / chunk_size)):
                ...
                try:
                    ...
                    logger.exception("Can't access the stack after address %s", stack_indicator)
                    break
...
class GetFrameInfo(gdb.Command):
    ...
    def invoke(self, arg, from_tty):
        ...
        if 0 <= int(frame_number) <= int(max_frame):
            ...
            logger.warning("Frame %s doesn't exist", frame_number)
            frame_info = None
